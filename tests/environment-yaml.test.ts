...
describe('serializeEnvironmentYaml', () => {
  ...
  it('emits disabled only for enabled=false and preserves descriptions', () => {
    const parsed = loadYaml(serializeEnvironmentYaml({
      name: 'env',
      values: [
  ...
  it('preserves only valid integer colors', () => {
    expect(loadYaml(serializeEnvironmentYaml({ name: 'valid', values: [], color: 359 })))
      .toEqual({ name: 'valid', values: [], color: 359 });
    for (const color of [-1, 360, 4.5, '5']) {
      expect(loadYaml(serializeEnvironmentYaml({ name: 'invalid', values: [], color })))
        .toEqual({ name: 'invalid', values: [] });
    }
  });

  it('defaults missing values array to empty', () => {
    expect(loadYaml(serializeEnvironmentYaml({ name: 'env' }))).toEqual({
      name: 'env',
      values: []
    });
  });
  ...
  it.each([{}, { name: '' }, null, undefined])(
    ...
    (body) => {
      expect(() => serializeEnvironmentYaml(body)).toThrow(/non-empty string name/);
    }
  );
...
describe('environmentFileName', () => {
  ...
  it('rejects C0 and C1 control characters before filename sanitization', () => {
    expect(() => environmentFileName('books-service', 'prod\u001fwest')).toThrow(
      /control characters/
    );
    expect(() => environmentFileName('books-service', 'prod\u0085west')).toThrow(
      /control characters/
  ...
  it('rejects malformed Unicode before filesystem encoding can collapse identities', () => {
    expect(() => environmentFileName('books-service', 'prod\ud800')).toThrow(
      /well-formed Unicode/
    );
    expect(() => environmentFileName('books-\ud801', 'prod')).toThrow(/well-formed Unicode/);
    expect(environmentFileName('books-service', 'prod-\ud83d\ude80')).toBe(
      'books-service - prod-🚀.environment.yaml'
    );
  });
...
describe('assertUniqueEnvironmentFileNames', () => {
  ...
  it('rejects collisions within preserved filenames', () => {
    expect(() => assertUniqueEnvironmentFileNames(
      'books-service',
      ['stage'],
      [
        'books-service - Prod.environment.yaml',
    ...
    )).toThrow(/same artifact filename/);
  });
});
...
describe('assertEnvironmentYamlRoundTrip', () => {
  ...
  it('rejects malformed or lossy candidates before promotion', () => {
    const expected = serializeEnvironmentYaml({
      name: 'env',
