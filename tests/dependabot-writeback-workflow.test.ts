...
describe('dependabot-writeback-workflow',()=>{
  ...
  it('permission boundaries',()=>{ expect(wf).toContain('contents: read'); expect(wf).toContain('contents: write'); expect(wf).toContain('pull-requests: write'); expect(wf).toContain('actions: read'); expect(wf).toContain('permissions:'); });
